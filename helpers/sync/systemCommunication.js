        ...
        this.socket.on("system", (incomingmessage) => {
            ...
            switch(action){
                ...
                case 'chat':    console.log('chat received');
                                caller.footer.current.handleState({popup:true, layer:false, msg:incomingmessage.params.text});
                                
                                setTimeout(()=>{
      
                                        caller.footer.current.handleState({popup:false})
                                    
                                },5000)

                                break;   

                case 'emoji':   console.log('emoji received');
                                caller.footer.current.handleState({emojiAnimation:true, emojiShow:false, image:incomingmessage.params.image});
                                
                                setTimeout(()=>{
      
                                        caller.footer.current.handleState({emojiAnimation:false})
